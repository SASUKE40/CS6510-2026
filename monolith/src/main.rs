...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let store = Store::open(&path, config.clone(), reset)?;
    let listener = tokio::net::TcpListener::bind(("127.0.0.1", port)).await?;
    println!(
        "Rust checkout monolith: http://{} database={path} {config:?}",
        listener.local_addr()?
    );
